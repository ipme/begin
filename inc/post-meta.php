...
function begin_entry_meta() {
	...
	if ( ! is_single() ) :
		if ( zm_get_option( 'meta_author' ) ) {
			if ( zm_get_option( 'author_hide' ) ) {
				echo '<span class="meta-author author-hide">';
			} else {
...
<span class="grid-inf gdz">
	<?php if ( has_post_format( 'link' ) ) { ?>
		<?php if ( get_post_meta( get_the_ID(), 'link_inf', true ) ) { ?>
			<span class="link-inf">
			<?php
			$link_inf = get_post_meta( get_the_ID(), 'link_inf', true );
			{ echo $link_inf;
			};
			?>
			</span>
		<?php } else { ?>
			<?php
			if ( zm_get_option( 'meta_author' ) && get_option( 'show_avatars' ) ) {
				?>
				<span class="grid-author"><?php grid_author_inf(); ?></span><?php } ?>
	...
	<span class="grid-inf-l">
		...
		if ( ! has_post_format( 'link' ) ) {
			?>
			<span class="date"><time datetime="<?php echo get_the_date( 'Y-m-d' ); ?> <?php echo get_the_time( 'H:i:s' ); ?>"><?php the_time( 'm/d' ); ?></time></span><?php } ?>
		<?php
		if ( get_post_meta( get_the_ID(), 'zm_like', true ) ) :
			?>
			<span class="grid-like"><span class="be be-thumbs-up-o">&nbsp;<?php zm_get_current_count(); ?></span></span><?php endif; ?>
	</span>
</span>
	<?php
}
		...
		switch ( $time_type ) {
			...
			case 'post';
				...
				if ( $time_diff <= 300 ) {
					...
					$time_string = str_replace( array_keys( $time_unit ), array_values( $time_unit ), $time_string );
					echo $time_string . sprintf( __( '前', 'begin' ) );
				} elseif ( ! get_bloginfo( 'language' ) === 'en-US' ) {
					echo the_time( 'm月d日' );
				} else {
					echo the_time( 'm/d' );
				}
