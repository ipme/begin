...
function be_mail_shortcode_init() {
	function be_mail_shortcode( $attr = array(), $content = null ) {
...
function bee_mail_shortcode_init() {
	function bee_mail_shortcode( $attr = array(), $content = null ) {
...
function be_display_contact_form() {
	...
	$content .= wp_nonce_field( 'be_mail_form', '_wpnonce', true, false );
	$content .= '<div class="mail-form-row">';
	$content .= bec_input( 'name', zm_get_option( 'mail_name' ), 'text', true, true, 'mail-form-col' );
	$content .= bec_input( 'email', zm_get_option( 'mail_email' ), 'email', true, true, 'mail-form-col' );
	if ( zm_get_option( 'mail_form_phone' ) ) {
		$content .= bec_input( 'phone', zm_get_option( 'mail_phone' ), 'tel', false, true, 'mail-form-col' );
	}
	if ( zm_get_option( 'mail_form_weixin' ) ) {
		$content .= bec_input( 'weixin', zm_get_option( 'mail_weixin' ), 'text', false, true, 'mail-form-col' );
	}
	$content .= '</div>';
	...
	$content .= be_help( $text = '主题选项 → 辅助功能 → 联系我们', $base = '辅助功能', $go = '联系我们' );
	$content .= '</div>';
	return $content;
}

// 输出表单
function bec_input( $name, $label, $type = 'text', $required = false, $has_label = true, $extra_class = '' ) {
	$content  = "<div class='item item-$name $extra_class'>";
...
// 输出内容表单
function bec_textarea( $name, $label, $required = false, $extra_class = '' ) {
	$content  = "<div class='item item-$name $extra_class'>";
...
function be_ajax_mail_message() {
	if ( ! isset( $_POST['type'] ) ) {
		wp_send_json_error( __( '无效请求', 'begin' ) );
		return;
	}
	$type = $_POST['type'];
	if ( $type !== '' ) {
		if ( $type === 'validation_error' ) {
			echo __( '请正确填写所有必填项。', 'begin' );
		}
		if ( $type === 'validation_phone_error' ) {
			echo __( '请输入一个有效的电话号码。', 'begin' );
		}
	}
	die();
}
...
function be_ajax_send_mail() {
	...
	if ( zm_get_option( 'mail_form_weixin' ) ) {
		$weixin = sanitize_text_field( $data['weixin'] );
	}
	...
	if ( zm_get_option( 'mail_form_weixin' ) ) {
		$message .= '<strong>' . __( '微信', 'begin' ) . ': </strong>' . $weixin . '<br />';
	}
