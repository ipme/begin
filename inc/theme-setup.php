...
if ( function_exists( 'register_sidebar' ) ) {
	...
	register_sidebar(
		array(
			...
			'before_widget' => '<div class="menucat"><div class="boxs3"><aside id="%1$s" class="widget %2$s" data-aos="">',
			'after_widget'  => '<div class="clear"></div></aside></div></div>',
			'before_title'  => '<h3 class="widget-title">',
			'after_title'   => '</h3>',
		)
	);
	...
	register_sidebar(
		array(
			...
			'id'            => 'group-three',
			'description'   => '显示在公司主页布局',
			'before_widget' => '<div class="xl3"><div class="boxs1"><aside id="%1$s" class="widget %2$s ms" data-aos="">',
			'after_widget'  => '<div class="clear"></div></aside></div></div>',
			'before_title'  => '<h3 class="widget-title">',
			'after_title'   => '</h3>',
		)
...
if ( ! is_array( cx_get_option( 'hide_wp_menu' ) ) || ! in_array( 'background', cx_get_option( 'hide_wp_menu' ) ) ) {
...
if ( zm_get_option( 'editor_style' ) ) {
	add_editor_style( '/inc/assets/css/editor-style.css' );
}
...
function block_editor_styles() {
	wp_enqueue_style( 'block-editor-style', get_theme_file_uri( '/inc/assets/css/editor-blocks.css' ), array(), version );
}
...
if ( is_admin() ) {
	function be_admin_css() {
		wp_enqueue_style( 'be-admin', get_template_directory_uri() . '/inc/assets/css/be-admin.css', array(), version );
	}
...
// 移除版本号
function be_remove_version( $src ) {
	if ( strpos( $src, '?ver=' ) ) {
		$src = remove_query_arg( 'ver', $src );
	}
	return $src;
}
if ( ! cx_get_option( 'remove_version' ) ) {
	add_filter( 'style_loader_src', 'be_remove_version', 10, 2 );
	add_filter( 'script_loader_src', 'be_remove_version', 10, 2 );
}
function begin_scripts() {
	wp_enqueue_style( 'begin-style', get_stylesheet_uri(), array(), version );
	wp_enqueue_style( 'be', get_template_directory_uri() . '/css/be.css', array(), version );
	wp_enqueue_style( 'super-menu', get_template_directory_uri() . '/css/super-menu.css', array(), version );
	...
	if ( is_array( zm_get_option( 'be_type' ) ) && in_array( 'sites', zm_get_option( 'be_type' ) ) ) {
		wp_enqueue_style( 'sites', get_template_directory_uri() . '/css/sites.css', array(), version );
	}
	...
	if ( zm_get_option( 'black_font' ) ) {
		wp_enqueue_style( 'black-font', content_url() . '/font/iconfont.css', array(), version );
	}

	if ( zm_get_option( 'color_icon' ) ) {
		wp_enqueue_style( 'color-icon', content_url() . '/icon/iconfont.css', array(), version );
		wp_enqueue_script( 'color-icon', content_url() . '/icon/iconfont.js', array(), version, true );
	}
	...
	if ( has_shortcode( get_post_field( 'post_content', get_the_ID() ), 'moveslider' ) ) {
		wp_enqueue_style( 'moveslider', get_template_directory_uri() . '/css/moveslider.css', array(), version );
	}

	if ( get_bloginfo( 'language' ) === 'en-US' ) {
		if ( cx_get_option( 'custom_fonts_en' ) ) {
			wp_enqueue_style( 'custom-fonts-en', cx_get_option( 'custom_fonts_en_url' ), array(), null );
		}
	} elseif ( cx_get_option( 'custom_fonts_cn' ) ) {
			wp_enqueue_style( 'custom-fonts-cn', cx_get_option( 'custom_fonts_cn_url' ), array(), null );
	}
	...
	if ( ! is_admin() ) {
		...
		$ajaxcontent .= 'var captcha = ' . wp_json_encode( array( 'verify' => zm_get_option( 'slider_captcha' ) ) ) . ';';
		$ajaxcontent .= 'var emilc = ' . wp_json_encode( array( 'ecy' => zm_get_option( 'go_reg' ) ) ) . ';';
		...
		if ( be_get_option( 'cms_collect' ) ) {
			$ajaxcontent .= 'var collect_new     = ' . wp_json_encode( array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) ) . ';';
			$ajaxcontent .= 'var collect_views   = ' . wp_json_encode( array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) ) . ';';
			$ajaxcontent .= 'var collect_comment = ' . wp_json_encode( array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) ) . ';';
			$ajaxcontent .= 'var collect_cat     = ' . wp_json_encode( array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) ) . ';';
			$ajaxcontent .= 'var collect_asset   = ' . wp_json_encode( array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) ) . ';';
			$ajaxcontent .= 'var collect_qa      = ' . wp_json_encode( array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) ) . ';';
		}

		wp_add_inline_script( 'be_script', $ajaxcontent, 'after' );

		$ajax  = 'var bea_ajax_params = ' . wp_json_encode(
			array(
				'bea_ajax_nonce' => wp_create_nonce( 'bea_ajax_nonce' ),
				'bea_ajax_url'   => admin_url( 'admin-ajax.php' ),
			)
		) . ';';
		...
		if ( zm_get_option( 'follow_btn' ) ) {
			$ajax .= 'var ajax_follow = ' . wp_json_encode( array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) ) . ';';
		}
		if ( is_plugin_active( 'erphpdown/erphpdown.php' ) ) {
			$ajax .= 'var ajax_checkin = ' . wp_json_encode( array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) ) . ';';
		}
		if ( zm_get_option( 'add_link' ) ) {
			$ajax .= 'var submit_link = ' . wp_json_encode( array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) ) . ';';
		}

		if ( zm_get_option( 'search_history' ) && zm_get_option( 'search_hot' ) ) {
			$ajax .= 'var ajax_searchhot = ' . wp_json_encode( array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) ) . ';';
		}

		if ( zm_get_option( 'emoji_show' ) ) {
			$ajax .= 'var ajax_smilies = ' . wp_json_encode( array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) ) . ';';
		}

		wp_add_inline_script( 'be_script', $ajax, 'after' );

		$falls_width = zm_get_option( 'falls_width' ) ?: '233';
		$fall        = 'var fallwidth = {fall_width: ' . $falls_width . '};';
		wp_add_inline_script( 'superfish', $fall, 'after' );

		wp_localize_script( 'superfish', 'assetsData', array( 'postID' => get_the_ID() ) );
		wp_localize_script( 'superfish', 'homeData', array( 'homeurl' => home_url() ) );
		...
		if ( zm_get_option( 'shar_poster' ) || zm_get_option( 'shar_share' ) || zm_get_option( 'qrurl' ) || ( is_array( zm_get_option( 'be_type' ) ) && in_array( 'sites', zm_get_option( 'be_type' ) ) ) ) {
	...
	if ( has_shortcode( get_post_field( 'post_content', get_the_ID() ), 'moveslider' ) ) {
		wp_enqueue_script( 'moveslider', get_template_directory_uri() . '/js/moveslider.js', array(), version, true );
	}
	...
	if ( is_singular() ) {
		...
		if ( zm_get_option( 'be_code' ) ) {
			if ( ! zm_get_option( 'be_code_css' ) || ( zm_get_option( 'be_code_css' ) == 'normal' ) ) {
				wp_enqueue_style( 'prettify-normal', get_template_directory_uri() . '/css/prettify-normal.css', array(), version );
			}
			if ( zm_get_option( 'be_code_css' ) == 'color' ) {
				wp_enqueue_style( 'prettify-color', get_template_directory_uri() . '/css/prettify-color.css', array(), version );
			}
