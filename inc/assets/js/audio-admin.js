jQuery(function($) {
     ...
     */
		function resetCoverImage($field) {
			...
			$coverWrap.removeClass(el.hasCoverClass).find("img").attr("src", "").attr("alt", "");
     ...
     */
		function resetField($field, hash) {
			...
			$field.find("input, textarea, select").not(":button").each(function() {
				var $this = $(this);
     ...
     */
		function getNewTrackField(hash, $container) {
			...
			var $clone = $parent.find(el.trackFieldClassName).first().clone().hide().show();
     ...
     */
		function setTrackFieldCover($field, cover) {
			...
			$coverField.find("img").attr("src", cover.url).attr("alt", cover.alt || "");

			$coverField.addClass(el.hasCoverClass).siblings("input").val(cover.id);
		}
     ...
     */
		el.$trackContainer.on("click", el.fieldHeadClassName,
		function(e) {
			var $this = $(this);
		...
		el.$expandAllButton.on("click",
		function(e) {
			var $this = $(this);
			var $container = $this.closest(".ai-container").find(".ai-fields-container");

			expandField($container.find(el.trackFieldClassName));
			e.preventDefault();
		});

		el.$collapseAllButton.on("click",
		function(e) {
			var $this = $(this);
			var $container = $this.closest(".ai-container").find(".ai-fields-container");
     ...
     */
		...
		el.$trackContainer.on("keyup", el.trackTitleClassName,
		function() {
			var $this = $(this);
		...
		/* Add Field Bottom */
		el.$addTrackButtonBottom.on("click",
		function() {
			var $this = $(this);
			var $container = $this.closest(".ai-container").find(".ai-fields-container");
		...
		/* Remove Track */
		el.$trackContainer.on("click", el.removeFieldButtonClassName,
		function() {
			var $this = $(this);
		...
		/* Remove All Tracks */
		el.$removeAllTracksButton.on("click",
		function() {
			var $this = $(this);
			var $container = $this.closest(".ai-container").find(".ai-fields-container");
     ...
     */
		...
		/* Audio upload */
		el.$trackContainer.on("click", el.audioUploadButtonClassName,
		function() {
			var $this = $(this);
     ...
     * as children
     */
		el.$trackContainer.on("click", el.coverUploadButtonClassName,
		function(e) {
			var $this = $(this);
			...
			wpMediaInit({
				...
				onMediaSelect: function(media) {
					setTrackFieldCover($this.parents(el.trackFieldClassName), {
						id: media.id,
						url: media.sizes.thumbnail ? media.sizes.thumbnail.url: media.sizes.full.url,
						alt: media.alt
					});
				}
			});
		...
		/* Remove Image */
		.on("click", el.coverRemoveClassName,
		function(e) {
			var $this = $(this);
     ...
     */
		el.$shortcodeInputField.on("click",
		function() {
			$(this).select();
		});
