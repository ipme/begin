...
if ( ! function_exists( 'be_build' ) ) {
	function be_build( $post_id, $meta_key = '', $default = null ) {

		$meta = get_post_meta( $post_id, 'be_build', true );
		return ( isset( $meta[ $meta_key ] ) ) ? $meta[ $meta_key ] : $default;
	}
}
...
if ( cx_get_option( 'build_metabox' ) ) {
	// 保存后的判断处理
	add_action(
		'save_post',
		function ( $post_id ) {
			...
			// 检查模板和清理数据
			if ( get_page_template_slug( $post_id ) !== 'pages/template-blocks.php' ) {
				delete_post_meta( $post_id, 'be_build' );
				return;
			}
	...
	ZMOP::createMetabox(
		...
		array(
			...
			'page_templates'     => array( 'pages/template-blocks.php' ),
			'post_formats'       => '',
			'show_restore'       => true,
			'output_css'         => true,
			'class'              => 'build-meta',
	...
	array(
		...
		'fields'      => array(
			...
			array(
				...
				'title'   => '面板简介',
				'type'    => 'content',
				'content' => '可以为页面添加与公司主页相同的模块，并分别调用不同页面的设置。',
			),
			...
			array(
				...
				'title'   => '使用方法',
				'type'    => 'content',
				'content' => '辅助设置 → 综合设置 → 文章面板 → 显示“构建面板”，新建页面，页面属性 → 模板 → 选择“模块构建”模板，添加操作与公司主页设置相同。',
			),
			...
			array(
				...
				'title'   => '重要提示',
				'type'    => 'content',
				'content' => '仅在使用时显示，用后关闭，否则未添加模块的页面也会存储额外的数据。',
			),
