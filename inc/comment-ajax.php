...
// 附加表单
$comment_country = ( isset( $_POST['country'] ) ) ? trim( strip_tags( $_POST['country'] ) ) : null;
$comment_phone   = ( isset( $_POST['phone'] ) ) ? trim( strip_tags( $_POST['phone'] ) ) : null;
// 结束
...
if ( zm_get_option( 'comment_info' ) && ! is_user_logged_in() ) {
	// 附加表单
	if ( zm_get_option( 'comment_country' ) && '' == $comment_country ) {
		err( '<i class="be be-info"></i>' . sprintf( __( '请填写', 'begin' ) ) . zm_get_option( 'country_text' ) . '。' );
	}

	if ( zm_get_option( 'comment_phone' ) && '' == $comment_phone ) {
		err( '<i class="be be-info"></i>' . sprintf( __( '请填写', 'begin' ) ) . zm_get_option( 'phone_text' ) . '。' );
	}
	// 结束
}
...
// 附加表单
update_comment_meta( $comment_id, 'country', $comment_country );
update_comment_meta( $comment_id, 'phone', $comment_phone );
// 结束
...
if ( ! $user->ID ) {
	...
	// 自定义表单
	setcookie( 'comment_country_' . COOKIEHASH, $comment_country ?: '', time() + $comment_cookie_lifetime, COOKIEPATH, COOKIE_DOMAIN );
	setcookie( 'comment_phone_' . COOKIEHASH, $comment_phone ?: '', time() + $comment_cookie_lifetime, COOKIEPATH, COOKIE_DOMAIN );
}
