...
function zm_add_style() {
	echo '<style type="text/css" media="screen">
		th.column-image {width:60px;}
		.form-field img.taxonomy-image {max-width:200px;max-height:40px;border-radius: 5px;}
		.inline-edit-row fieldset .image label span.title, .column-image span {width:40px;height:40px;display:inline-block;overflow: hidden;border-radius: 5px;}
		.inline-edit-row fieldset .image img,.column-image img {width: auto;height: 40px;}
	</style>';
}
	...
	return '<script type="text/javascript">
		jQuery(document).ready(function($) {
			...
			$(".editinline").click(function() {	
			    ...
			    var image = $("#tag-"+tax_id+" .image img").attr("src");

				if (image != "' . ZM_IMAGE_PLACEHOLDER . '") {
					$(".inline-edit-col :input[name=\'taxonomy_image\']").val(image);
				} else {
					$(".inline-edit-col :input[name=\'taxonomy_image\']").val("");
				}
				
				$(".inline-edit-col .title img").attr("src",image);
			});
	    });
	</script>';
}
...
function zm_quick_edit_custom_box( $column_name, $screen, $name ) {
	if ( $column_name == 'image' ) {
		echo '<fieldset>
		<div class="image inline-edit-col">
			<label>
