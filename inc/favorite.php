...
class Begin_Favorite_Posts {
	...
	function display_favorites( $post_type = 'all', $user_id = false, $limit = 10, $show_remove = true ) {
		...
		if ( $posts ) {
			...
			$remove_link = ' <a href="#" data-id="%s" title="%s" class="keep-remove-favorite"><i class="be be-cross"></i></a>';
...
class begin_favorite_post_widget extends WP_Widget {
	...
	function form( $instance ) {
		...
		$exclude_post_types = array( 'attachment', 'surl' );
		...
		<p>
			<label class="alignleft" style="display: block; width: 50%; margin-bottom: 10px;">文章类型：
			<select id="<?php echo $this->get_field_id( 'post_type' ); ?>" name="<?php echo $this->get_field_name( 'post_type' ); ?>" >
				<option value="all" <?php selected( $post_type, '全部' ) ?>>全部</option>
				<?php foreach ($post_types as $pt) { ?>
					<option value="<?php echo $pt; ?>" <?php selected( $post_type, $pt ) ?>><?php echo isset( $post_type_names[$pt] ) ? $post_type_names[$pt] : $pt; ?></option>
				<?php } ?>
			</select>
			</label>
		</p>
		<p>
			<label class="alignleft" style="display: block; width: 50%; margin-bottom: 10px;" for="<?php echo $this->get_field_id( 'limit' ); ?>">文章数量：
				<input class="number-text-be" id="<?php echo $this->get_field_id( 'limit' ); ?>" name="<?php echo $this->get_field_name( 'limit' ); ?>" type="number" step="1" min="1" value="<?php echo $instance['limit']; ?>" size="3" />
			</label>
		</p>
		<p>
