...
if ( 'POST' == $_SERVER['REQUEST_METHOD'] && ! empty( $_POST['action'] ) && $_POST['action'] == 'update-user' ) {
	...
	if ( ! empty( $_POST['userimg'] ) ) {
		wp_update_user(
			array(
				'ID'      => $current_user->ID,
				'userimg' => esc_attr( $_POST['userimg'] ),
			)
		);
	}
	...
	if ( ! empty( $_POST['weixinqr'] ) ) {
		wp_update_user(
			array(
				'ID'       => $current_user->ID,
				'weixinqr' => esc_attr( $_POST['weixinqr'] ),
			)
		);
	}
...
<div class="profile-box">
	<h4 class="update-user-t user-info-ico"><?php _e( '我的资料', 'begin' ); ?></h4>
	<div id="my-profile" class="user-profile" role="main">
		<?php if ( is_user_logged_in() ) { ?>
			<div class="profile-message">
				...
				<?php
				if ( count( $error ) > 0 ) {
					echo '<p class="profile-error">' . implode( '<br />', $error ) . '</p>';}
				?>
			</div>
			<p class="clear"></p>
			...
			<form name="profile" id="your-profile" action="" method="post" autocomplete="off" enctype="multipart/form-data">
				...
				<?php
				$current_user = wp_get_current_user(); if ( empty( $current_user->user_email ) ) {
					?>
					bind-tip"<?php } ?>">
					...
					<p class="form-avatar load">
						...
						<?php if ( get_option( 'show_avatars' ) ) { ?>
							<div class="my-gravatar-apply">
								<?php
								if ( ! zm_get_option( 'avata_url' ) || ( zm_get_option( 'avata_url' ) == 'weavatar.com/avatar/' ) ) {
									$gravatarurl = 'https://weavatar.com/';
								}

								if ( zm_get_option( 'avata_url' ) == 'cravatar.cn/avatar/' ) {
