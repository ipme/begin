...
get_header(); ?>
	<?php begin_primary_class(); ?>
		...
		<main id="main" class="be-main site-main<?php indent(); ?>
		<?php
		if ( zm_get_option( 'code_css' ) ) {
			?>
			code-css<?php } ?>
			<?php
			if ( get_post_meta( get_the_ID(), 'sub_section', true ) ) {
				?>
			sub-h<?php } ?>" role="main">
							...
							<div class="tab-down-wrap">
								...
								<div class="tab-down-content">
									...
									<div class="tab-content-item tab-content-license">
										<p>
											<?php
											$down_explain = cx_get_option( 'down_explain' );
											echo ! empty( $down_explain ) ? wp_kses_post( stripslashes( $down_explain ) ) : '';
											?>
										</p>
										<div class="clear"></div>
									</div>
