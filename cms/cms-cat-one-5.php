...
if ( be_get_option( 'cat_one_5' ) ) { ?>
	<div class="line-one line-one-no-5 betip">
		<?php if ( be_get_option( 'cat_one_5_id' ) ) { ?>
			<?php
				...
				$tax_terms = get_terms( $tax , array(
					'include' => explode( ',', be_get_option( 'cat_one_5_id' ) ),
					'orderby' => 'include',
					'order'   => 'ASC',
				));
			...
			<?php } else { ?>
				<div class="cat-container ms" <?php aos_a(); ?>>
					...
					<div class="cms-cat-area">
						<ul class="cat-one-list">
							<li class="list-cat-title srm">首页设置 → 杂志布局 → 单栏分类列表(5篇文章)</li>
						</ul>
						<div class="clear"></div>
					</div>
				</div>
			<?php } ?>
		<?php cms_help( $text = '首页设置 → 杂志布局 → 单栏分类列表(5篇文章)', $number = 'cat_one_5_s', $go = '单栏分类列表5篇文章' ); ?>
	</div>
<?php } ?>