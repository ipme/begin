...
// 分类问答列表
jQuery(document).ready(function($) {
	$(document).on('click', '.qa-item .qa-title',
	function(e) {
		e.preventDefault();
		var $item = $(this).closest('.qa-item');
		var $allItems = $('.qa-item');
		if ($item.hasClass('qa-expanded')) {
			$item.removeClass('qa-expanded');
		} else {
			$allItems.removeClass('qa-expanded');
			$item.addClass('qa-expanded');
			$('html,body').animate({
	...
	$(document).on('click', '.qa-item .qa-title a',
	function(e) {
		var $item = $(this).closest('.qa-item');
		if (!$item.hasClass('qa-expanded')) {
			e.preventDefault();
			$('.qa-item').removeClass('qa-expanded');
			$item.addClass('qa-expanded');
			$('html,body').animate({
				scrollTop: $item.offset().top - 100
			},
	...
	$(document).on('click', '.qa-item .qa-content',
	function(e) {
		e.stopPropagation();
	});
});