...
<?php if ( co_get_option( 'group_cat_hot' ) ) {
	if ( ! co_get_option( 'hot_bg' ) || ( co_get_option( 'hot_bg' ) == 'auto' ) ) {
		$bg = '';
	}
	if ( co_get_option( 'hot_bg' ) == 'white' ) {
		$bg = ' group-white';
	}
	if ( co_get_option( 'hot_bg' ) == 'gray' ) {
		$bg = ' group-gray';
	}
?>
	<?php $categories = explode( ',', co_get_option( 'group_cat_hot_id' ) );
	...
	?>
		...
		<div class="betip line-group-cat-hot g-row g-line notext<?php echo $bg; ?>" <?php aos(); ?>>
			<div class="g-col">
				...
				<?php co_help( $text = '公司主页 → 热门分类', $number = 'group_cat_hot_s', $go = '热门分类' ); ?>
			</div>
		</div>
	<?php } ?>
<?php } ?>