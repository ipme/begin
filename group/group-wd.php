...
<?php if ( co_get_option( 'group_wd' ) ) {
	if ( ! co_get_option( 'wd_bg' ) || ( co_get_option( 'wd_bg' ) == 'auto' ) ) {
		$bg = '';
	}
	if ( co_get_option( 'wd_bg' ) == 'white' ) {
		$bg = ' group-white';
	}
	if ( co_get_option( 'wd_bg' ) == 'gray' ) {
		$bg = ' group-gray';
	}
?>
	...
	?>
		<div class="g-row grl-img g-line<?php echo $bg; ?>" <?php aos(); ?>>
			<div class="g-col">
				...
				<?php co_help( $text = '公司主页 → 分类左右图', $number = 'group_wd_s', $go = '分类左右图' ); ?>
				<div class="clear"></div>
			</div>
		</div>
	<?php } ?>
