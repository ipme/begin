...
<?php if ( co_get_option( 'group_counter' ) ) { ?>
	<div class="line-counter g-row" style="background: url('<?php echo co_get_option( 'group_counter_bg' ); ?>') no-repeat fixed center / cover;" <?php aos(); ?>>
		<div class="g-col">
			<div class="group-counter-box">
				...
				<?php } ?>
				<?php co_help( $text = '公司主页 → 计数器', $number = 'counter_s', $go = '计数器' ); ?>
			</div>
		</div>
	</div>
<?php } ?>