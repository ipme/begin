...
<?php if ( co_get_option( 'group_cat_a' ) ) {
	if ( ! co_get_option( 'cat_a_bg' ) || ( co_get_option( 'cat_a_bg' ) == 'auto' ) ) {
		$bg = '';
	}
	if ( co_get_option( 'cat_a_bg' ) == 'white' ) {
		$bg = ' group-white';
	}
	if ( co_get_option( 'cat_a_bg' ) == 'gray' ) {
		$bg = ' group-gray';
	}
?>
<div class="g-row g-line group-cat-a notext<?php echo $bg; ?>" <?php aos(); ?>>
	<div class="g-col">
		<div class="group-cat">
			<?php $display_categories =  explode( ',', co_get_option( 'group_cat_a_id' ) );
		...
		<?php co_help( $text = '公司主页 → 新闻资讯A', $number = 'group_cat_a_s', $go = '新闻资讯A' ); ?>
	</div>
</div>
<?php } ?>