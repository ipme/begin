			...
			<div id="navigation-top" class="navigation-top">
				<div id="site-nav-wrap-o">
					...
					<nav id="site-nav" class="main-nav-o<?php nav_ace(); ?>">
						<?php menu_nav(); ?>
						<?php be_help( $text = '主题选项 → 菜单设置 → 通用头部模式', $base = '菜单设置', $go = '通用头部模式' ); ?>
					</nav>
					<div class="clear"></div>
				...
				<div class="site-nav-wrap-o-r betip">
					...
					<?php be_help( $text = '主菜单登录按钮', $base = '站点管理', $go = '管理站点' ); ?>
				</div>
			</div>
		</div>
	</div>
...
<?php if ( ! zm_get_option( 'menu_m' ) || ( zm_get_option( 'menu_m' ) == 'menu_d' ) ){ ?>
	<div id="header-main" class="header-main<?php if ( zm_get_option( 'top_nav_show' ) && zm_get_option( 'top_nav_ganged' ) ) { ?> top-nav-ganged<?php } ?><?php if ( zm_get_option( 'top_nav_show' ) ) { ?> top-nav-show<?php } ?>">
<?php } ?>
<?php if (zm_get_option('menu_m') == 'menu_n'){ ?>
	<div id="header-main-n" class="header-main-n">
<?php } ?>
<?php if (zm_get_option('menu_m') == 'menu_g'){ ?>
	<div id="header-main-g" class="header-main-g<?php if ( zm_get_option( 'top_nav_ganged' ) ) { ?> top-nav-ganged<?php } ?><?php if ( zm_get_option( 'top_nav_show' ) ) { ?> top-nav-show<?php } ?>">
<?php } ?>
		<?php if ( zm_get_option( 'top_nav_show' ) ) { ?>
				...
				<div class="be-nav-box">
					<div class="be-nav-l">
						<div class="be-nav-logo">
							...
							<?php be_help( $text = '主题选项 → 网站标志', $base = '', $go = '网站标志' ); ?>
						</div>

						<?php if ( cx_get_option( 'weather_widget' ) && cx_get_option( 'weather_mode' ) == 'logo' ) { ?>
						...
						<?php if ( ! zm_get_option( 'main_menu' ) || ( zm_get_option( 'main_menu' ) == 'main_menu_l' ) ) { ?>
							<div class="be-nav-wrap">
								...
								<?php be_help( $text = '主题选项 → 菜单设置 → 菜单外观', $base = '菜单设置', $go = '菜单外观' ); ?>
							</div>
						<?php } ?>
					</div>
