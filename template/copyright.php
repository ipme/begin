					...
					<?php if ( get_bloginfo( 'language' ) === 'en-US' ) { ?>
						<?php 
							if ( ! empty( get_the_author_meta( 'last_name' ) ) ) {
								echo '<a href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ), get_the_author_meta( 'user_nicename' ) ) ) . '" rel="external nofollow">' . esc_html( get_the_author_meta( 'last_name' ) ) . '</a>';
							}
						?>
					<?php } else { ?>
					<?php echo str_replace( '<a', '<a rel="external nofollow"', get_the_author_posts_link() ); ?>
					<?php } ?>
				</strong>
				<?php _e( '投稿', 'begin' ); ?>，<?php _e( '发表于', 'begin' ); ?> <?php time_ago( $time_type ='posts' ); ?>
			</li>
		<?php } else { ?>
...
<div class="authorbio ms load betip" <?php aos_a(); ?>>
		...
		<?php else: ?>
			<?php if ( zm_get_option('copyright_statement') == '' ) { ?>
				<li>
					...
					<?php _e( '发表于', 'begin' ); ?><?php time_ago( $time_type ='posts' ); ?>
				</li>
			<?php } else { ?>
			<li class="reprinted"><?php echo str_replace( array('{{title}}', '{{link}}' ), array( get_the_title(), get_permalink() ), stripslashes( zm_get_option( 'copyright_statement' ) ) ); ?></li>
			<?php } ?>
	...
	<?php be_help( $text = '主题选项 → 文章设置 → 文章信息 → 正文底部版权信息', $base = '文章设置', $go = '文章信息' ); ?>
	<div class="clear"></div>
</div>
<?php endif; ?>