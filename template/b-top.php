...
<div class="cms-news-grid-container b-top">
		...
		<?php if ( has_post_format( 'link' ) ) { ?>
			...
			<header class="entry-header">
				...
				<?php $direct = get_post_meta( get_the_ID(), 'direct', true ); ?>
					<h2 class="entry-title over"><a href="<?php echo $direct ?>" target="_blank" rel="external nofollow" <?php echo goal(); ?>><?php the_title(); ?></a></h2>
				<?php } else { ?>
					<h2 class="entry-title over"><a href="<?php the_permalink(); ?>" rel="bookmark" <?php echo goal(); ?>><?php the_title(); ?></a></h2>
				<?php } ?>
			</header>
	...
	<?php sh_help( $text = '首页设置 → 博客布局 → 其它模块 → 推荐文章', $number = '', $base = '博客布局', $go = '其它模块' ); ?>
	<div class="clear"></div>
</div>
<?php } ?>