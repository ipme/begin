...
<div class="grid-cat-box grid-cat-top" <?php aos_a(); ?>>
				...
				<div class="boxs1">
					<div class="grid-cat-bx4 grid-cat-new ms<?php echo fill_class(); ?>"<?php echo be_img_fill(); ?>>
						<figure class="picture-img">
							...
							<?php if ( has_post_format( 'video' ) ) { ?><div class="play-ico"><a rel="bookmark" <?php echo goal(); ?> href="<?php echo esc_url( get_permalink() ); ?>"><i class="be be-play"></i></a></div><?php } ?>
						...
						<?php the_title( sprintf( '<h2 class="grid-title"><a href="%s" rel="bookmark" ' . goal() . '>', esc_url( get_permalink() ) ), '</a></h2>' ); ?>
	...
	<?php sh_help( $text = '首页设置→ 分类图片→ 其它模块 → 推荐文章', $number = '', $base = '分类图片', $go = '其它模块' ); ?>
	<div class="clear"></div>
</div>