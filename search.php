...
?>
<?php if ( zm_get_option( 'search_cat_name' ) ) { ?>
		...
		<div class="cat-rec-main cat-rec-4">
			<a href="<?php echo esc_url( get_term_link( $term ) ); ?>" rel="bookmark" <?php echo goal(); ?>>
				<div class="cat-rec-content tra ms" <?php aos_a(); ?>>
					...
					<h4 class="cat-rec-title"><?php echo esc_html( $term->name ); ?></h4>
...
<!-- list -->
	<section id="primary" class="content-area search-site<?php if ( $search_count == '1' ) { ?> search-one<?php } ?><?php if ( zm_get_option('infinite_post' ) ){ ?> search-list-infinite<?php } ?><?php if ( zm_get_option( 'search_sidebar' ) ) { ?> search-sidebar<?php } ?>">
...
<!-- normal -->
	<section id="primary" class="content-area search-normal<?php if ( $search_count == '1' ) { ?> search-one<?php } ?><?php if ( zm_get_option( 'search_sidebar' ) ) { ?> search-sidebar<?php } ?>">
		<?php if ( have_posts() ) : ?>
