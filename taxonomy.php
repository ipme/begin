...
<?php wp_body_open(); ?>
<div id="page" class="hfeed site<?php page_class(); ?><?php if ( zm_get_option( 'body_width' ) == 'percent' ) { ?> percent<?php } ?><?php if ( zm_get_option( 'be_debug' ) ) { ?> debug<?php } ?><?php if ( ! zm_get_option( 'turn_small' ) ) { ?> nav-normal<?php } ?>">
...
<?php do_action( 'be_header_sub' ); ?>
	...
	<?php if ( ! zm_get_option( 'filter_layout' ) || ( zm_get_option( 'filter_layout' ) == 'img' ) ) { ?>
								...
								<div class="picture-box ms<?php echo fill_class(); ?>"<?php echo be_img_fill(); ?>>
									<figure class="picture-img">
										<?php if ( zm_get_option( 'hide_box' ) ) { ?>
											...
											<a rel="bookmark" href="<?php echo esc_url( get_permalink() ); ?>"><div class="hide-excerpt">
											<?php
											if ( has_excerpt( '' ) ) { echo wp_trim_words( get_the_excerpt(), 62, '...' );
											} else { echo wp_trim_words( get_the_content(), 72, '...' ); }
											?>
											</div></a>
										<?php } ?>
	...
	<?php if ( zm_get_option( 'filter_layout' ) == 'default' ) { ?>
	...
	<?php if ( zm_get_option( 'filter_layout' ) == 'list' ) { ?>
		<section id="category-list" class="content-area category-list">
			<main id="main" class="be-main site-main domargin" role="main">
				...
				<?php if ( have_posts() ) : ?>
					...
					<?php while ( have_posts() ) : the_post(); ?>
						<article id="post-<?php the_ID(); ?>" class="post-item-list post scl" <?php aos_a(); ?>>
