...
<section id="note" class="content-area note-area">
	...
	<main id="main" class="be-main site-main note-main begd<?php if ( zm_get_option( 'note_main_overall' ) ) { ?> all<?php } ?><?php indent(); ?><?php if ( zm_get_option( 'code_css' ) ) { ?> code-css<?php } ?><?php if ( get_post_meta( get_the_ID(), 'sub_section', true ) ) { ?> sub-h<?php } ?>" role="main">
				...
				<div class="entry-content">
					...
					<div class="single-content">
						...
						<?php if ( zm_get_option( 'note_notice' ) ) { ?>
							<div class="note-notice betip">
								<?php echo wpautop( zm_get_option( 'note_notice' ) ); ?>
								<?php be_help( $text = '主题选项 → 分类模板 → 常规模式 → 文档模板设置 → 底部固定内容' ); ?>
							</div>
						<?php } ?>
