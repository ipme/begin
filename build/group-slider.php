...
<?php if ( be_build( get_the_ID(), 'group_slider' ) ) {
	$slider = ( array ) be_build( get_the_ID(), 'slider_group' );
	foreach ( $slider as $items ) {
		$slider_count = count( $slider );
	}
?>
<div class="g-row slider-row">
	<?php if ( ! be_build( get_the_ID(), 'group_slider_video' ) ) { ?>
				...
				<?php if ( ! empty( $items['slider_group_img'] ) ) { ?>
					<div class="slider-group-main<?php if ( $items['slider_scale'] ) { ?> bigease<?php } ?>">
						<a href="<?php echo $items['slider_group_url']; ?>" rel="bookmark" <?php echo goal(); ?>>
							<div class="group-big-img big-back-img<?php if ( isset( $items['group_blur'] ) && $items['group_blur'] ) { ?> big-blur<?php } ?>" style="background-image: url('<?php echo $items['slider_group_img']; ?>');height:<?php echo be_build( get_the_ID(), 'big_back_img_h' ); ?>px;"></div>
						</a>
							...
							<?php if ( $items['slider_group_video'] ) { ?>
								<div class="group-small-video-area group-act2">
									<div class="slider-video-area">
										<video src="<?php echo $items['slider_group_video']; ?>" poster="<?php echo be_build( get_the_ID(), 'group_slider_video_img' ); ?>" autoplay="" loop="" muted="muted" playsinline="" controlslist="nodownload"></video>
									</div>
								</div>
	...
	<?php } else { ?>
		<div class="group-slider-video-box slider-videos no-controls-video<?php if ( be_build( get_the_ID(), 'group_slider_video_mask' ) ) { ?> video-mask<?php } ?>">
			<video class="slider-video" src="<?php echo be_build( get_the_ID(), 'group_slider_video_url' ); ?>" poster="<?php echo be_build( get_the_ID(), 'group_slider_video_img' ); ?>" autoplay="" loop="" muted="muted" playsinline="" controlslist="nodownload"></video>
			<div class="owl-carousel group-video-text">
				<?php 
					$text = ( array ) be_build( get_the_ID(), 'group_video' );
	...
	<?php if ( be_build( get_the_ID(), 'group_slide_progress' ) && ! be_build( get_the_ID(), 'group_slider_video' ) && ! be_build( get_the_ID(), 'group_only_img' ) && $slider_count !== 1 ) { ?><div class="slide-mete"><div class="slide-progress" style="-webkit-transition: width <?php echo be_get_option( 'owl_time' ); ?>ms;transition: width <?php echo be_get_option( 'owl_time' ); ?>ms;"></div></div><?php } ?>
</div>
<?php } ?>