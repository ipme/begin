...
function confirmExit() {
	...
	if (hasChanged || (mce && !mce.isHidden() && mce.isDirty())) {
		return bet_messages.unsaved_changes_warning
	}
}
...
jQuery(document).ready(function($) {
	...
	$("td.post-delete a").click(function(event) {
		...
		var confirmation = confirm(bet_messages.confirmation_message);
		if (!confirmation) {
			return
		}
		$(this).hide();
	...
	$("#bet-featured-image a#bet-featured-image-link").click(function(e) {
		e.preventDefault();
		custom_uploader = wp.media.frames.file_frame = wp.media({
			title: bet_messages.media_lib_string,
			button: {
				text: bet_messages.media_lib_string
			},
			multiple: false
		});
		...
		function() {
			...
			jQuery("#bet-featured-image input#bet-featured-image-id").val(attachment.id);
			$.ajax({
				type: "POST",
				url: betajaxhandler.ajaxurl,
				data: {
					action: "bet_fetch_featured_image",
					img: attachment.id
				},
				success: function(data, textStatus, XMLHttpRequest) {
					$("#bet-featured-image-container").html(data);
					hasChanged = true
				},
				error: function(MLHttpRequest, textStatus, errorThrown) {
					alert(errorThrown)
