function captcha_script() {
	jQuery(document).ready(function($) {
		const CAPTCHA_EXPIRE_KEY = 'BE_captcha_expire';
		const COUNTDOWN_SECONDS = 60;

		function startCountdown(captcha, expireTime) {
			function updateCountdown() {
				...
				if (remaining <= 0) {
					...
					localStorage.removeItem(CAPTCHA_EXPIRE_KEY);
					return;
				}
				
				captcha.addClass("disabled");
				captcha.html('重新获取 -' + remaining + '');
				setTimeout(updateCountdown, 1000);
			}
			updateCountdown();
		}

		const savedExpireTime = parseInt(localStorage.getItem(CAPTCHA_EXPIRE_KEY));
		if (savedExpireTime && savedExpireTime > Date.now()) {
			$('.be-email-code').addClass("disabled");
			startCountdown($('.be-email-code'), savedExpireTime);
		}
			...
			if (captcha.hasClass("disabled")) {
				...
				function(data) {
					if ($.trim(data) == "1") {
						...
						const expireTime = Date.now() + COUNTDOWN_SECONDS * 1000;
						localStorage.setItem(CAPTCHA_EXPIRE_KEY, expireTime);
						startCountdown(captcha, expireTime);
					} else if ($.trim(data) == "3") {
